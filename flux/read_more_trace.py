...
def analyze_trace(events):
    ...
    # First pass: collect GPU user annotations
    for event in events:
        if event.get('cat') == 'gpu_user_annotation' and event.get('ph') == 'X':
            ts = event.get('ts', 0) // 10  # Round to 10 nanosecond precision
            gpu_annotations[ts] = event.get('name', 'Unknown')

    # Second pass: process operators and match with annotations
    for event in events:
        if event.get('ph') == 'X':  # Complete events
            ...
            # Check for matching GPU annotation
            if ts in gpu_annotations:
                name = f"{gpu_annotations[ts]}:{name}"
...
'''
Usage: python read_more_trace.py <path_to_chrome_trace.json>
python profile/read_more_trace.py /home/nvme-share/home/lurunyu/projects/xDiT/flux/profile_data/ulysses_2_ring_2/xfuser_flux_trace_steps_20_rank_0.json > log.log
'''

if __name__ == "__main__":
    # import sys
    # if len(sys.argv) != 2:
    #     print("Usage: python script.py <path_to_chrome_trace.json>")
    #     sys.exit(1)
    # main(sys.argv[1])
