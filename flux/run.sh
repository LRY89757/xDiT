      ...
      for rd in "${ring_degrees[@]}"; do
        for size in "${sizes[@]}"; do
          ...
          if [ "$product" -eq "$pd" ]; then
            ...
            --pipefusion_parallel_degree 1 --num_pipeline_patch 1 --batch_size $bs --use_flops_profile --no_profiler 2>&1 | tee logs/flux/log_flops_size${size}_ulysses${ud}_ring${rd}_bs${bs}_pd${pd}.log"
            echo $cmd
            eval $cmd
          fi
        done
